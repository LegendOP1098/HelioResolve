...
def _looks_like_dataset_root(path: Path) -> bool:
    if not path.exists():
        return False
    split_names = {alias for aliases in SPLIT_ALIASES.values() for alias in aliases}
    return any((path / split / "low_res").exists() and (path / split / "high_res").exists() for split in split_names)
...
def find_dataset_root(start: str | Path | None = None) -> Path:
    ...
    dataset_root = next((candidate for candidate in candidates if _looks_like_dataset_root(candidate)), None)
    if dataset_root is None:
        raise FileNotFoundError(
            "Could not find the solar dataset. Expected either "
            "a dataset root with split/low_res and split/high_res folders, "
