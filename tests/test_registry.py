...
def test_model_registry_contains_expected_models() -> None:
    assert {"srcnn", "resm", "rcan", "swinir", "diffusion_sr"}.issubset(set(list_available_models()))
...
def test_resm_forward_shape_with_solar_features() -> None:
    model = build_model("resm", in_channels=10, out_channels=1, scale=4, capacity="tiny")
    output = model(torch.rand(1, 10, 8, 8))
    assert tuple(output.shape) == (1, 1, 32, 32)
